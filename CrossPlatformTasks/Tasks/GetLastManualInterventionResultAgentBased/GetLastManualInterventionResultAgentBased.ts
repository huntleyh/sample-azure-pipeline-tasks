...
async function run() {
    try {
        ...
        let executablePSPath: string = path.join(__dirname, 'worker.ps1');

        console.log('Launching TypeScript startup');
        console.log('Found executablePSPath=',executablePSPath);
        
        let statusVariableName = tl.getInput('statusVariableName', true);
        let commentVariableName = tl.getInput('commentVariableName', true);

        let scriptArguments = ` -statusVariableName '${statusVariableName}' -commentVariableName '${commentVariableName}'`;
